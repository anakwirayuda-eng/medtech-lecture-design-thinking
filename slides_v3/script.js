(function () {
    ...
    function closeZoomedImages() {
        document.querySelectorAll(".evidence-img.zoomed").forEach((img) => {
            img.classList.remove("zoomed");
        });
        document.body.classList.remove("image-zoom-mode");
    }

    function hasZoomedImage() {
        return document.querySelector(".evidence-img.zoomed") !== null;
    }
    ...
    function changeSlide(direction) {
        setSlide(current + direction);
    }

    window.changeSlide = changeSlide;

    document.addEventListener("keydown", (event) => {
        if (event.key === "Escape" && hasZoomedImage()) {
    ...
    document.querySelectorAll(".evidence-img").forEach((img) => {
        img.addEventListener("click", (event) => {
            ...
            if (!wasZoomed) {
                ...
                document.body.classList.add("image-zoom-mode");
            }
        });
    });

    document.addEventListener("click", (event) => {
        if (!event.target.classList.contains("evidence-img")) {
            closeZoomedImages();
        }
    });
